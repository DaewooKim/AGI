...
## 파국적 망각 회피
- replay: 학습 데이터를 새로운 학습 시 함께 제시하여 학습 분포를 유지
- 정규화: 모델 파라미터가 만히 변하지 않도록 제약을 검
- 파라미터 격리: 새로운 지식을 완전히 별도 파라미터에 저장. 기존 파라미터는 건드리지 않음 (e.g. LoRA)

## 망각 전략
- 메모리에 오래 쌓인 정보끼리 상호 충돌을 일으켜 모델이 혼란
- semantic forgetting 기법: 오래되었고 잘 참조되지 않은 벡터 임베딩에 점수를 매겨 제거
- 잊어버릴 것은 과감히 지워야 새로운 정보가 메모리에 자리잡을 수 있고 검색 효율을 유리
- 업데이트: 이전 메모리 삭제, "과거" 같은 flag를 담
...
- 해마 (빠른 학습 시스템)
  - 개별 사건의 구체적인 내용을 신속하고 자동으로 인코딩하는데 특화됨
- 신피질 (느린 학습 시스템)
  - 구조화된 지식과 통계적 규칙성을 점진적으로 습득하는데 특화됨. 일반화에는 이상적이지만 너무 빠르게 학습이 일어나면 파국적 간섭에 매우 취약
...
# 향후 연구 방향
## 새로운 메모리 구조 검색
- tree 구조 기억, 지리 공간 기억(procedural memory을 의미하는 듯)
- 미래에는 자기조작화 메모리 --> 모델이 어떤 구조로 데이터를 저장할지 스스로 학습해 가는 형태

## 확장성과 효율성
- 기억에 대한 서열화 및 중요도 재정렬
- 더 발전되고 확장 가능한 KG 데이터베이스와 더 효율적인 통합 전략을 연구 (e.g. active learning)

## 지식 통합과 모델 업데이트
- 장기적으로 메모리에 쌓인 지식을 모델 자체에 통합
- 외부 메모리가 비대해지는 것을 막고 모델의 내재적 능력을 향상
- 잘못 학습된 편향이나 불필요 지식을 모델 안에서 제거
- Machine unlearning, Re-training

## 메모리 신뢰성 및 사실 검증
- LLM 스스로 메모리를 주기적으로 재검토
- 외부 지식베이스와 대조하여 메모리의 사실 여부 --> 오류가 발견되면 표시하거나 제거

## 다중 모드성
- mLLM의 일화적 버퍼 및 KG 구성 능력을 이미지, 오디오 및 기타 데이터 양식을 처리하도록 확장 

## 에이전트적 공진화
- iLLM과 mLLM이 보다 정교한 에이전트 루프에서 자신의 능력을 적극적으로 공진화시키는 "선순환" 개념을 더 깊이 연구
